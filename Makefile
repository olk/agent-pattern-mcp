# =============================================================================
# Makefile for agent-pattern-mcp
# =============================================================================
...
.PHONY: help install install-mcps \
	...
	test-unit test-oracles test-mutations regen-mutmut-baseline \
...
DOCKER_IMAGE    := agent-pattern-mcp
DOCKER_TAG      := $(shell grep -m 1 '^version' pyproject.toml | sed -E 's/.*"([^"]+)".*/\1/')
DOCKER_HUB_REPO := olkowa/agent-pattern-mcp
GHCR_REPO       := ghcr.io/olk/agent-pattern-mcp
...
# a gate must not mutate the working tree. Run `uv run ruff check --fix .` /
# `uv run ruff format .` directly when wanted.
...
##@ Tests
# Coverage flags are an agent-pattern-mcp local-dev requirement (review
# amendment D1): the [tool.coverage] fail_under=85 ratchet must stay enforceable
# from the developer entry point, not only from CI's direct pytest invocation.
test-unit: install ## Run unit tests with coverage (tests/unit/)
	$(UV) run pytest tests/unit/ -v --cov=src --cov-branch --cov-report=term-missing
...
# the shim and this pin in the same change.
#
# Pytest tmp isolation: every pytest process under this gate pins its own
# per-PID --basetemp under MUTMUT_PYTEST_TMP (consumed by
# verify/mutmut/mutmut_compat.py's pytest_configure; same EXIT-trap lifecycle
# as HYPO_DIR) so parallel children never race the shared tmp root.
#
...
test-mutations: ## L3: mutmut over the Tier A scope + gardens (manual/nightly; ephemeral install via uv)
	@bash -o pipefail -c '\
	HYPO_DIR=$$(mktemp -d); \
...
# comparison activates via AGENT_CONSISTENCY_MODEL, checker identity pinned
...
##@ Demo
client: ## Run the agent-patterns MCP client demo
	@AGENT_CLIENT_URL=http://localhost:8061/mcp uv run python examples/agent_client.py

client-async: ## Run the async job trio demo (submit_agent_design_job + get_agent_design_status + cancel_agent_design)
	@AGENT_CLIENT_URL=http://localhost:8061/mcp uv run python examples/agent_client_async.py
...
docker-logs: ## Show docker compose logs (one-time)
	$(COMPOSE) logs agent-pattern-mcp
