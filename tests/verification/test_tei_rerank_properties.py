...
def _patch_transport(status_code: int, body: Any) -> AbstractContextManager[object]:
    ...
    # agent-pattern-mcp's _safe_tei_rerank_call consumes the client as a
    # context manager ("with httpx.Client() as client") — wire __enter__ to
    # return the fake client itself so the patched constructor seam holds.
    fake_client.__enter__ = lambda: fake_client  # type: ignore[method-assign]
    fake_client.__exit__ = lambda *a: None  # type: ignore[method-assign]
    return mock.patch(
