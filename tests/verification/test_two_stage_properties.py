...
``tests/unit/test_two_stage_fixes.py`` — the L1 behavioural oracle).
  ...
  TS-3 (T5)  ``RequirementWeights`` exposes exactly the seven
...
from src.pipeline import AgentPatternPipeline, AnalysisResult
...
N_QA = len(QUALITY_ATTRIBUTE_KEYS)


class _ConfigOnlyPipeline(AgentPatternPipeline):
...
def _weights(first: float, *rest: float) -> RequirementWeights:
    return RequirementWeights(
        reliability=first,
        cost_efficiency=rest[0],
        latency=rest[1],
        output_quality=rest[2],
        observability=rest[3],
        safety=rest[4],
        simplicity=rest[5],
    )
...
class TestTS1EmptyAnalysisTolerated:
    @given(
        qm=st.builds(
            ...
            cost_efficiency=qa_value,
            latency=qa_value,
            output_quality=qa_value,
            observability=qa_value,
            safety=qa_value,
        ),
    )
    ...
    def test_empty_lists_never_raise_and_are_preserved(self, qm: QualityMetrics) -> None:
        ...
        assert metrics.reliability == qm.reliability
...
class TestTS2ScoredPatternRoundtrip:
    ...
    def test_scores_survive_json_roundtrip(
        ...
        sp = ScoredPattern(
            ...
            benefits=["b"],
            tradeoffs=["t"],
            quality_attributes=dict.fromkeys(QUALITY_ATTRIBUTE_KEYS, 5.0),
...
class TestTS3WeightKeysComplete:
    def test_requirement_weights_exactly_the_qa_keys(self) -> None:
        assert len(QUALITY_ATTRIBUTE_KEYS) == 7
        w = _weights(0.5, 0.4, 0.3, 0.2, 0.1, 0.0, 0.25)
        assert set(w.as_dict()) == set(QUALITY_ATTRIBUTE_KEYS)

    @given(
        w1=weight_value, w2=weight_value, w3=weight_value,
        w4=weight_value, w5=weight_value, w6=weight_value, w7=weight_value,
    )
    @settings(max_examples=20, deadline=None)
    def test_as_dict_roundtrips_every_weight(
        self, w1: float, w2: float, w3: float,
        w4: float, w5: float, w6: float, w7: float,
    ) -> None:
        w = _weights(w1, w2, w3, w4, w5, w6, w7)
        d = w.as_dict()
        assert d["reliability"] == w1
        assert d["simplicity"] == w7
...
class TestTS4ZeroWeightsUnweightedMean:
    ...
    def test_zero_weights_fall_back_to_unweighted_mean(
        ...
        zero_weights = _weights(*(0.0,) * N_QA)
        ...
        expected = round(ref_blend(vals, [1.0] * N_QA) * 10.0, 2)  # uniform weights
...
class TestTS5WeightedAverageDifferential:
    @given(
        weights=st.tuples(
            ...
            weight_value, weight_value, weight_value, weight_value, weight_value, weight_value,
        ),
        qa=st.dictionaries(st.sampled_from(list(QUALITY_ATTRIBUTE_KEYS)), qa_value),
    )
...
class TestTS6SelectionOrdering:
    ...
    def test_sorted_by_blended_score_when_blending_active(
        ...
        pipeline = _ConfigOnlyPipeline()
        patterns = [
            _pattern_with_qa(f"p{i}", {"reliability": a, "safety": b})
            for i, (a, b) in enumerate(rows)
        ]
        scored = pipeline._score_patterns(  # noqa: SLF001
            patterns, _weights(0.5, 0.5, 0.0, 0.5, 0.0, 0.5, 0.5)
        )
    ...
    def test_sorted_by_analysis_score_when_blending_off(self) -> None:
        ...
        patterns = [
            _pattern_with_qa("low", {"reliability": 1.0, "safety": 1.0}),
            _pattern_with_qa("high", {"reliability": 9.0, "safety": 9.0}),
        ]
        scored = pipeline._score_patterns(  # noqa: SLF001
            patterns, _weights(0.5, 0.5, 0.0, 0.5, 0.0, 0.5, 0.5)
        )
