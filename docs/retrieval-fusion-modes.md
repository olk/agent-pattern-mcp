...
- Removing the `"rerank"` alternative eliminates the
  ...
  caused 100% fallback to react on default values
...
- **`min_fusion_score` gate** (default 0.0, range `[0, 2/60]`) →
  `react` fallback tagged `is_fallback=True`. Guarded against
...
- Internal issue #3 — `min_fusion_score` fragility under rank_fusion (see
