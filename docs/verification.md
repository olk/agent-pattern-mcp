# Verification — agent-pattern-mcp
...
| L9 | in-repo corpus + structural invariants (`tests/eval/`) | well-formedness and internal consistency of LLM output: section completeness, reference closure, producer+consumer pairs, catalogue existence, score ranges | `llm` marker + `AGENT_BENCH_LLM=1` (live); offline vacuity guard always on | active |
...
- `make verify-cross-consistency` — the NL-Doc gate (VLP evidence: validating
  ...
  docstring); the LLM comparison activates via `AGENT_CONSISTENCY_MODEL`, with
...
through the real pipeline under `AGENT_BENCH_LLM`; the tracked pass-rate is
...
AGENTS.md (required-review paths and the verification-program rules), the perf smoke canary
...
| `AGENT_BENCH_LLM=1 pytest tests/eval/ -m llm` | L9 | prompt/model changes |
