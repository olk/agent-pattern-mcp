# Testing Strategies — agent-pattern-mcp (The Layered Testing Program)
...
|---|---|---|
| P1 | **Decorrelated oracles.** Layers are chosen so their blind spots do not overlap; residuals multiply instead of add (`docs/verification.md` §6.1) | The §2 stack: type system / sampling / mutation / interleaving / proof are orthogonal bug-class owners |
...
| L9 | LLM-output contract corpus | in-repo `tests/eval/` corpus + structural invariant assertions (run under `AGENT_BENCH_LLM`) | **structural well-formedness of LLM output** (the T1.5 slice): cited pattern names exist, component/event reference closure, producer+consumer pairs, score ranges, section completeness; prompt/model drift detection | min (LLM API) | advisory drift gate on prompt/model changes | VLP intent-layer evidence; owned by §3.9a — never judges design quality (T2) |
...
(T2) — the agent system designs themselves are judged by evaluation rubrics
and human review, never by a theorem or a test. That boundary is the core
claim of `docs/verification.md` and is repeated here because testing
...
The third exploration cell (`docs/verification.md` §2.7): seeded
...
- **NL-Doc cross-consistency gate** (`make verify-cross-consistency`,
  docs/verification.md §3.1 VLP entry): a *second* LLM summarizes each
...
(opt-in via the existing `AGENT_BENCH_LLM` marker — never in default CI) and
...
- required sections/fields of `AgentSystemDesign` are present and non-empty.
...
(coarse benchmark smoke, docs/verification.md §4.5 — proofs guarantee
...
| **Week 0** (first amendment — before Phase 0) | **guarded-transition bug fix in `src/tools/jobs.py` + unit tests** (the fix-before-proof sequencing correction: `J-1`/`J-2` become true of the implementation before any `.fizz` model or DST experiment claims them — docs/verification.md §4.5). **Second amendment (E6/E4):** the same Week-0 window adds the constructor-injectable `JobsStore` lock (E6) and the E4-extended secret canary (W0-3) — the full task list with acceptance criteria is [`week-0-task-breakdown.md`](week-0-task-breakdown.md) |
...
| L9 | — | in-repo `tests/eval/` corpus harness (`AGENT_BENCH_LLM`; no external framework) |
...
perf/llm markers opt-in via `RUN_PERF`/`AGENT_BENCH_LLM`.
...
**Companion documents:**
- Rationale and tool landscape: [`docs/verification.md`](docs/verification.md)
