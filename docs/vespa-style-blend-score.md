...
- Removes the interaction between selection-mode × `min_fusion_score`-scale that previously caused 100% fallback to react-pattern on defaults
...
After slug-cut survivors are resolved to full patterns, a second blend combines the **requirements-weighted analysis score** with a **min-max-normalized fusion score** to produce the final `blended_score` used for pattern ranking and `recommended_topology` selection:
...
|-------------------|----------------|-------------|-------------------|--------------------------|
| supervisor-worker | 90.0           | 0.0250      | 75.0              | 0.7×90 + 0.3×75 = 85.5  |
| react             | 65.0           | 0.0300      | 100.0             | 0.7×65 + 0.3×100 = 75.5 |
| self-rag         | 50.0           | 0.0100      | 0.0               | 0.7×50 + 0.3×0 = 35.0   |
...
Blend: 0.7·analysis_score + 0.3·fusion_normalized
    ...
    ▼
blended_score ∈ [0, 100]
recommended_topology = topology of top-scoring pattern (gated by topology_score_threshold);
                       pattern name surfaced separately as recommended_pattern_name
```

**Note on naming:** `DomainMatch.fusion_score` carries the **stage-1 slug-cut blend value** (`RR(fused) + RR(ce)`, scale `[0, 0.033]`), while the scored pattern's `blended_score` carries the **stage-2 selection blend value** (`0.7·analysis + 0.3·fusion_normalized`, scale `[0, 100]`). They are different metrics at different pipeline stages.
...
| Concern | Location |
|---------|----------|
| Stage-1 blend formula + `RRF_K` | `src/patterns/retriever.py :: RRF_K`, `reciprocal_rank_score` |
| Stage-1 blend application (slug-cut) | `src/patterns/retriever.py :: HybridPatternRetriever.retrieve` |
| Stage-2 blend formula + weights | `src/pipeline.py :: _score_patterns` |
| Blend weight config fields | `src/config.py :: RetrievalConfig` (`analysis_blend_weight`, `fusion_blend_weight`, `weight_smoothing_alpha`) |
| `RANK_FUSION_BLEND_MAX = 2/60` | `src/config.py :: RANK_FUSION_BLEND_MAX` |
