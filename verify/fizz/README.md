...
| `jobs_runner.fizz` | the background task lifecycle + cancel tool (`src/tools/submit_architecture_design.py::_run_job`, `src/tools/cancel_agent_design.py`): W0-1 checkpoints, terminal-write races, task-map bookkeeping | J-1 (kill authority), RUN-1, RUN-3, RUN-4, C-1 |
...
| `design_loop.fizz` | `AgentPatternPipeline.design_loop` triage decisions (`src/pipeline.py:1015`): best-score guard, early stop, cancel checkpoint, malformed-continue | DL-2, DL-3, DL-4, DL-5 |
